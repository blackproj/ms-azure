...
import sys
import requests
from get_token.auth import generate_access_token, GRAPH_API_ENDPOINT
...
def get_stats():
	endpoint = GRAPH_ENDPOINT + '/me/drive'
	response = requests.get(endpoint, headers = headers)

	if response.ok:
		data = response.json()
		print('Username:', data['owner']['user']['displayName'], 
...
def get_root():
	endpoint = GRAPH_ENDPOINT + '/me/drive/root'
	response = requests.get(endpoint, headers = headers)
...
def get_root_list(bool, root_id, endpoint):
	print()
	while bool == True:
		# all_files_id contains id of files
		all_files_id = list_files(endpoint)				
		chosen_id = choose_file(all_files_id)
		bool_value = which_kind(chosen_id)

		# simple file - no folder here
		if(bool_value == True):								
			download_content(chosen_id)
			bool = False
		# file selected is a folder
		else:
			bool_nested = is_dir_empty(chosen_id)	
			bool_nested = True
			endpoint_nested = GRAPH_ENDPOINT + '/me/drive/items/' + chosen_id + '/children'
			get_root_list(bool_nested, root_id, endpoint_nested)

# list the root folder of my cloud onedrive business
...
def is_dir_empty(single_file_id):
	endpoint = GRAPH_ENDPOINT + '/me/drive/items/' + single_file_id
	response = requests.get(endpoint, headers = headers)
	data = response.json()
...
def which_kind(single_file_id):
	endpoint = GRAPH_ENDPOINT + '/me/drive/items/' + single_file_id + '?$select=file'
	response = requests.get(endpoint, headers = headers)
	tmp = []
	data = response.json()
	tmp.append(data)
...
def download_content(single_file_id):
	endpoint = GRAPH_ENDPOINT + f'/me/drive/items/' + single_file_id
	pwd_dir = os.getcwd()
	save_location = os.path.join(pwd_dir, r'User_downloads')
